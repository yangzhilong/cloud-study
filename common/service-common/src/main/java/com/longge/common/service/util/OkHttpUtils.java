...
public class OkHttpUtils {
    ...
    private static Request processPostJsonParameter(String url, Map<String, String> headerParameter, Object bodyObj) {
        ...
        if(!Objects.isNull(bodyObj)) {
            RequestBody body = RequestBody.Companion.create(JSONObject.toJSONString(bodyObj), MediaType.parse("application/json; charset=utf-8"));
            builder.post(body);
        } else {
            RequestBody body = RequestBody.Companion.create("{}", MediaType.parse("application/json; charset=utf-8"));
            builder.post(body);
        }
        if (!isEmptyMap(headerParameter)) {
            builder.headers(Headers.of(headerParameter));
